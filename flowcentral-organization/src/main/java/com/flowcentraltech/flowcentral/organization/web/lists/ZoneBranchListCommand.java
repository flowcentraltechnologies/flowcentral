...
/**
 * Zone branch list command.
 * 
 * @author FlowCentral Technologies Limited
 * @since 4.1
 */
@Component("zonebranchlist")
public class ZoneBranchListCommand extends AbstractOrganizationListCommand<LongParam> {

    public ZoneBranchListCommand() {
    ...
    public List<? extends Listable> execute(Locale locale, LongParam params) throws UnifyException {
        if (params.isPresent()) {
            return org().findMappedBranches((MappedBranchQuery) new MappedBranchQuery().zoneId(params.getValue())
