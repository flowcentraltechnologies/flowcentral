...
/**
 * Applet alerts configuration.
 * 
 * @author FlowCentral Technologies Limited
 * @since 4.1
 */
@JsonInclude(value = Include.NON_NULL, content = Include.NON_EMPTY)
public class AppletAlertsConfig extends BaseConfig {
    
    @JacksonXmlElementWrapper(useWrapping = false)
    @JacksonXmlProperty(localName = "alert")
    private List<AppletAlertConfig> alertList;

    public AppletAlertsConfig(List<AppletAlertConfig> alertList) {
        this.alertList = alertList;
    }

    public AppletAlertsConfig() {
