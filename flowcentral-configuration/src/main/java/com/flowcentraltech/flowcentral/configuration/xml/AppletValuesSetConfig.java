...
/**
 * Applet values set configuration.
 * 
 * @author FlowCentral Technologies Limited
 * @since 4.1
 */
@JsonInclude(value = Include.NON_NULL, content = Include.NON_EMPTY)
public class AppletValuesSetConfig extends BaseConfig {

    @JacksonXmlElementWrapper(useWrapping = false)
    @JacksonXmlProperty(localName = "values")
    private List<AppletSetValuesConfig> valuesList;

    public AppletValuesSetConfig(List<AppletSetValuesConfig> valuesList) {
        this.valuesList = valuesList;
    }

    public AppletValuesSetConfig() {
