...
/**
 * Workflow values set configuration.
 * 
 * @author FlowCentral Technologies Limited
 * @since 4.1
 */
@JsonInclude(value = Include.NON_NULL, content = Include.NON_EMPTY)
public class WfValuesSetConfig extends BaseConfig {

    @JacksonXmlElementWrapper(useWrapping = false)
    @JacksonXmlProperty(localName = "setValues")
    private List<WfSetValuesConfig> setValuesList;

    public WfValuesSetConfig(List<WfSetValuesConfig> setValuesList) {
        this.setValuesList = setValuesList;
    }

    public WfValuesSetConfig() {
