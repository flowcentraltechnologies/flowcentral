...
/**
 * Entity unique constraints configuration.
 * 
 * @author FlowCentral Technologies Limited
 * @since 4.1
 */
@JsonInclude(value = Include.NON_NULL, content = Include.NON_EMPTY)
public class EntityUniqueConstraintsConfig extends BaseConfig {
    
    @JacksonXmlElementWrapper(useWrapping = false)
    @JacksonXmlProperty(localName = "uniqueConstraint")
    private List<EntityUniqueConstraintConfig> uniqueConstraintList;

    public EntityUniqueConstraintsConfig(List<EntityUniqueConstraintConfig> uniqueConstraintList) {
        this.uniqueConstraintList = uniqueConstraintList;
    }

    public EntityUniqueConstraintsConfig() {
