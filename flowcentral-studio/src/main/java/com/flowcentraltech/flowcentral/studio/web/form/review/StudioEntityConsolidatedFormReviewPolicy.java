...
/**
 * Studio entity consolidated form review policy.
 * 
 * @author FlowCentral Technologies Limited
 * @since 1.0
 */
@EntityReferences({ "application.entity" })
@Component("studioentity-consolidatedformreview")
public class StudioEntityConsolidatedFormReviewPolicy extends AbstractStudioConsolidatedFormReviewPolicy {
    ...
    protected void review(ValueStore instValueStore, FormReviewType reviewType, TargetFormMessages messages)
        ...
        if (FormReviewType.ON_DELETE.equals(reviewType)) {
            if (usageListProvider.countUsages(instValueStore.getReader(), null) > 0) {
                messages.addError("$m{studio.entity.has.usages}", "usages");
            }
        }
    }
