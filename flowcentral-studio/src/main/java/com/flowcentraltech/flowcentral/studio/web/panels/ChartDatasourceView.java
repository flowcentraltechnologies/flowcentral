...
public class ChartDatasourceView extends AbstractStudioEditorPage {
            ...
            try {
                if (categories != null && categories.length > 0) {
                    ...
                    final EntityDef.Builder edb = EntityDef.newBuilder(ConfigType.CUSTOM,
                            "com.flowcentraltech.flowcentral.preview.Az", "Preview", null, null, null, null, null,
                            false, false, false, false, false, "application.preview", "Preview", 1L, 1L);
                    edb.addFieldDef("application.text", "application.text", EntityFieldDataType.STRING,
                            EntityFieldType.STATIC, "category", "Category");
                    ...
                    if (category.getSeries() != null && category.getSeries().length > 0) {
                        for (CDSnapshotSeries series : category.getSeries()) {
                            ...
                            if (grouping == 0) {
                                edb.addFieldDef("application.decimal", "application.decimal",
                                        EntityFieldDataType.DECIMAL, EntityFieldType.CUSTOM, series.getNm(),
                                ...
                                edb.addFieldDef("application.datetime", "application.datetime",
                                        EntityFieldDataType.TIMESTAMP, EntityFieldType.CUSTOM, series.getNm(),
                                        series.getLbl() != null ? series.getLbl() : series.getNm());
                            }
                        }
                    }
                    ...
                    final TableDef.Builder tdb = TableDef.newBuilder(entityDef, "Preview", false, false,
                            "studio.snapshotTable", "Priview Table", 0L, 0L);
                    tdb.itemsPerPage(-1);
                        ...
                        if (_category.getSeries() != null && _category.getSeries().length > 0) {
                            if (_category.getSeries()[0] != null) {
                                ...
                                for (int i = 0; i < len; i++) {
                                    ...
                                    map.put("category",
                                            _category.getLbl() != null ? _category.getLbl() : _category.getNm());
    ...
    public boolean isPresent() {
        return chartDatasourceName != null;
    }
}
