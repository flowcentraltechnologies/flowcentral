...
/**
 * Studio applet form list command
 * 
 * @author FlowCentral Technologies Limited
 * @since 1.0
 */
@Component("studioappletinputformlist")
public class StudioAppletInputFormListCommand extends AbstractApplicationListCommand<LongParam> {

    public StudioAppletInputFormListCommand() {
    ...
    public List<? extends Listable> execute(Locale locale, LongParam longParam) throws UnifyException {
        if (longParam.isPresent()) {
            ...
            if (!StringUtils.isBlank(entity)) {
                return au().getApplicationEntitiesListables(new AppFormQuery().type(FormType.INPUT).entity(entity));
            }
        }
