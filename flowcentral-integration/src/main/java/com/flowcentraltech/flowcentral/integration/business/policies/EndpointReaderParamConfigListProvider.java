...
/**
 * End-point reader parameter configuration list provider.
 * 
 * @author FlowCentral Technologies Limited
 * @since 1.0
 */
@Component(IntegrationModuleNameConstants.ENDPOINTREADER_PARAMCONFIGPROVIDER)
public class EndpointReaderParamConfigListProvider extends AbstractParamConfigListProvider {
    ...
    public String getCategory(Entity entityInst) throws UnifyException {
        return "integration-readconfig";
    }

    @Override
    public List<ParamConfig> getParamConfigList(Entity entityInst) throws UnifyException {
        String endpointReaderName = entityInst != null ? ((ReadConfig) entityInst).getEndpointReader() : null;
        if (!StringUtils.isBlank(endpointReaderName)) {
            return integrationModuleService.getEndpointReaderParamConfigs(endpointReaderName);
        }
