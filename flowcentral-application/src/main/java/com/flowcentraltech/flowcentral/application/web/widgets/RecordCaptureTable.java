...
public class RecordCaptureTable<T extends AbstractRecordCapture> {
    ...
    public void clear(List<String> titles) throws UnifyException {
        if (!DataUtils.isBlank(titles)) {
            records = new ArrayList<T>(titles.size());
            for (String title: titles) {
                T record = (T) ReflectUtils.newInstance(tableDef.getRecordClass());
