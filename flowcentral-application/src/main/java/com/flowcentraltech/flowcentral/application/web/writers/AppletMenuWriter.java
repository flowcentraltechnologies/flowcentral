...
public class AppletMenuWriter extends AbstractMenuWriter {
	...
	protected void doWriteSectionStructureAndContent(ResponseWriter writer, Widget widget, String sectionId)
		...
		if (appletMenuWidget.getMenuSectionId().equals(sectionId)) {
			...
			ApplicationAllPaths allpaths = (ApplicationAllPaths) getApplicationAttribute(
					AppletApplicationAttributeConstants.APPLICATION_PATHS_ALL);
			if (allpaths == null) {
				synchronized (this) {
					allpaths = (ApplicationAllPaths) getApplicationAttribute(
							AppletApplicationAttributeConstants.APPLICATION_PATHS_ALL);
					if (allpaths == null) {
						...
						for (ApplicationMenuDef applicationMenuDef : applicationDefList) {
							for (AppletDef appletDef : applicationMenuDef.getAppletDefList()) {
								paths.add(appletDef.getOpenPath());

								if (appletDef.isWithOpenDraftPath()) {
									draftAppletList.add(appletDef);
									paths.add(appletDef.getOpenDraftPath());
								}
							}
						}

						setApplicationAttribute(AppletApplicationAttributeConstants.APPLICATION_PATHS_ALL,
								allpaths = new ApplicationAllPaths(paths));
					}
				}
			} else {
				for (ApplicationMenuDef applicationMenuDef : applicationDefList) {
			...
			final boolean horizontal = appletMenuWidget.isHorizontal();
			if (horizontal) {
				writer.write("<div class=\"hmslide\" style=\"left:0px;\" id=\"").write(appletMenuWidget.getSlideId())
						.write("\">");
			}

			final boolean isGetRolePaths = !StringUtils.isBlank(roleCode) && !allpaths.isWithRolePaths(roleCode);
			List<String> rolePaths = isGetRolePaths ? new ArrayList<String>() : Collections.emptyList();
			for (ApplicationMenuDef applicationMenuDef : applicationDefList) {
