...
public class BaseApplicationEntityPolicy extends BaseConfigNamedEntityPolicy {
    ...
    public Object preCreate(Entity record, Date now) throws UnifyException {
        if (FlowCentralEditionConstants.ENTERPRISE.equalsIgnoreCase(getContainerSetting(String.class,
            ...
            ((BaseApplicationEntity) record).setVersionType(VersionType.NEW);
        } else {
            ((BaseApplicationEntity) record).setVersionType(VersionType.CURRENT);
        }
    ...
    public void preUpdate(Entity record, Date now) throws UnifyException {
        if (!FlowCentralEditionConstants.ENTERPRISE.equalsIgnoreCase(getContainerSetting(String.class,
                FlowCentralContainerPropertyConstants.FLOWCENTRAL_INSTALLATION_TYPE))) {
            ((BaseApplicationEntity) record).setRootMergeVersionNo(null);
        }
