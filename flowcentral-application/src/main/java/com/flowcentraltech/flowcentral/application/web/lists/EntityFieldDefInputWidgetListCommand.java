...
public class EntityFieldDefInputWidgetListCommand extends AbstractApplicationListCommand<EntityFieldDefListParams> {
    ...
    public EntityFieldDefInputWidgetListCommand() {
    ...
    public List<? extends Listable> execute(Locale locale, EntityFieldDefListParams params) throws UnifyException {
        if (params.isPresent()) {
            EntityFieldDef entityFieldDef = params.getEntityDef().getFieldDef(params.getFieldName());
            EntityFieldDataType fieldType = entityFieldDef.getDataType();
            if (EntityFieldDataType.LIST_ONLY.equals(fieldType)) {
                fieldType = entityFieldDef.getResolvedTypeFieldDef().getDataType();
            }
